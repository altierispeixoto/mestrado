...
class Trapezoidal():
    ...
    def calculate_membership(self, x):
        ...
        if self.x_min <= x and x <= self.x_top_left:
            try:
                return (self.height *
                        ((x - self.x_min) /
                            (self.x_top_left - self.x_min))).quantize(
                                                  gs.DECIMAL_ROUNDING)
            ...
            try:
                return (self.height *
                        ((self.x_max - x) /
                            (self.x_max - self.x_top_right))).quantize(
                                                   gs.DECIMAL_ROUNDING)
            except ZeroDivisionError:
                # the base and top are the same resulting in a vertical line
                return self.height
    ...
    def calculate_alpha_cut(self, alpha):
        ...
        right_point = (self.x_max +
                       ...
                       (alpha / self.height))
        return (left_point.quantize(gs.DECIMAL_ROUNDING),
                right_point.quantize(gs.DECIMAL_ROUNDING))
