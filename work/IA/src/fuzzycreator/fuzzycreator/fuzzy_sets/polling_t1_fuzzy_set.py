...
class PollingT1FuzzySet():
    ...
    def calculate_membership(self, x):
        ...
        try:
            ...
            if self.interp_method is LINEAR:
                ...
                slope = (yr - yl) / (xr - xl)
                return Decimal(slope * (x - xl) + yl).quantize(gs.DECIMAL_ROUNDING)
            else:
                y = self.lagrange_poly(float(x))  # will not work with Decimal
                return Decimal(str(y)).quantize(gs.DECIMAL_ROUNDING)
                #return y
    ...
    def calculate_alpha_cut(self, alpha):
        ...
        x_values = list(linspace(gs.global_uod[0], gs.global_uod[1],
                                 gs.global_x_disc))
        ...
        for i in range(len(x_values)):
            if self.calculate_membership(x_values[i]) >= alpha:
                if not previous_hit:
                    results.append(Decimal(x_values[i])
                                   .quantize(gs.DECIMAL_ROUNDING))
                    previous_hit = True
            else:
                if previous_hit:
                    results.append(Decimal(x_values[i-1])
                                   .quantize(gs.DECIMAL_ROUNDING))
                previous_hit = False
        if len(results) == 1:
        ...
        if len(results) == 2:
            return results
        return zip(results[0::2], results[1::2])
