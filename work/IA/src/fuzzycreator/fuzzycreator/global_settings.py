...
def get_y_points():
    ...
    return [(Decimal(i)/global_alpha_disc)
            .quantize(DECIMAL_ROUNDING)
            for i in range(global_alpha_disc, 0, -1)]


def get_z_points():
    """Get a list of discretised zLevels."""
    return [(Decimal(i)/global_zlevel_disc)
            .quantize(DECIMAL_ROUNDING)
...
def get_z_level_heatmap():
    ...
    return list(Color('blue').range_to(Color('red'), global_zlevel_disc))


def get_z_level_greyscale():
    """Get a list of hex colours for the heat map of zSlices."""
    return list(Color('#BBBBBB').range_to(Color('#222222'),
                global_zlevel_disc))
