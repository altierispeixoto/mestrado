...
class PollingTest(unittest.TestCase):
    ...
    def test_vertical(self):
        ...
        self.assertEquals(fs1.calculate_membership(1.5), Decimal('0.75'))
        self.assertEquals(fs1.calculate_membership(7), Decimal('0'))
    ...
    def test_alpha_cut(self):
        ...
        points = {0: 0.1, 1: 0.5, 2: 1, 3: 0.5, 4: 1, 5: 0.5, 6: 0.1}
        fs1 = PollingT1FuzzySet(points)
        self.assertEquals(fs1.calculate_alpha_cut(1), [(2, 2), (4, 4)])
        self.assertEquals(fs1.calculate_alpha_cut(Decimal('0.75')),
                          [(Decimal('1.5000'), Decimal('2.5000')),
                           (Decimal('3.5000'), Decimal('4.5000'))])
        self.assertEquals(fs1.calculate_alpha_cut(Decimal('0.5')), [1, 5])
        fs2 = FuzzySet(Gaussian(5, 1))
        self.assertEquals(distance_t1.mcculloch(fs1, fs2), 2)
