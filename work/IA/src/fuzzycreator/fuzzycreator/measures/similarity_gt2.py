...
def _zslice_jaccard(fs1, fs2, z):
    ...
    if j_bottom == 0:
        return Decimal(0)
    return (j_top / j_bottom).quantize(gs.DECIMAL_ROUNDING)
...
def zhao_crisp(fs1, fs2):
    ...
    return (sim / gs.global_zlevel_disc).quantize(gs.DECIMAL_ROUNDING)
...
def yang_lin(fs1, fs2):
    ...
    if n == 0:
        return 0
    return (result / n).quantize(gs.DECIMAL_ROUNDING)
...
def mohamed_abdaala(fs1, fs2):
    ...
    return (result / gs.global_x_disc).quantize(gs.DECIMAL_ROUNDING)
...
def hung_yang(fs1, fs2):
    ...
    for x in gs.get_x_points():
        ...
        try:
            ...
            pass
    return (1 - (distance / gs.global_x_disc)).quantize(gs.DECIMAL_ROUNDING)
