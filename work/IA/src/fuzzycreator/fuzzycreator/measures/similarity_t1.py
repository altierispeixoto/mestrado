...
def pappis1(fs1, fs2):
    ...
    return (1 - dist).quantize(gs.DECIMAL_ROUNDING)
...
def pappis2(fs1, fs2):
    ...
    for x in gs.get_x_points():
        ...
        dist1 += abs(y1 - y2)
        dist2 += y1 + y2
    return (1 - (dist1 / dist2)).quantize(gs.DECIMAL_ROUNDING)
...
def pappis3(fs1, fs2):
    ...
    for x in gs.get_x_points():
        ...
        dist += abs(y1 - y2)
        n += 1
    return (1 - (dist / n)).quantize(gs.DECIMAL_ROUNDING)
...
def jaccard(fs1, fs2):
    ...
    for x in gs.get_x_points():
        ...
        sim1 += min(y1, y2)
        sim2 += max(y1, y2)
    return (sim1 / sim2).quantize(gs.DECIMAL_ROUNDING)
...
def zwick(fs1, fs2):
    ...
    return sim.quantize(gs.DECIMAL_ROUNDING)
...
def chen(fs1, fs2):
    ...
    for x in gs.get_x_points():
        ...
        fs1_squares += (y1 * y1)
        fs2_squares += (y2 * y2)
    return (top / max(fs1_squares, fs2_squares)).quantize(gs.DECIMAL_ROUNDING)
