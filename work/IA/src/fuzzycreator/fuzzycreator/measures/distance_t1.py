...
def ralescu1(fs1, fs2):
    ...
    return Decimal(a).quantize(gs.DECIMAL_ROUNDING)
...
def grzegorzewski_non_inf_pq(fs1, fs2, p=2, q=0.5):
    ...
    return distance.quantize(gs.DECIMAL_ROUNDING)
...
def grzegorzewski_non_inf_p(fs1, fs2, p=2):
    ...
    return max(left_dist, right_dist).quantize(gs.DECIMAL_ROUNDING)
...
def grzegorzewski_inf_q(fs1, fs2, q=0.5):
    ...
    return (left_dist + right_dist).quantize(gs.DECIMAL_ROUNDING)
...
def ban(fs1, fs2):
    ...
    return ((left_dist + right_dist) **
            Decimal('0.5')).quantize(gs.DECIMAL_ROUNDING)
...
def allahviranloo(fs1, fs2, c=0.5, f=lambda a: a):
    ...
    return sqrt(centre_difference +
                width_difference).quantize(gs.DECIMAL_ROUNDING)
...
def yao_wu(fs1, fs2):
    ...
    return (Decimal('0.5') * Decimal(a)).quantize(gs.DECIMAL_ROUNDING)
...
def _get_directional_distance(cut1, cut2):
    ...
    if isinstance(cut1[0], Decimal) and isinstance(cut2[0], Decimal):
        ...
        return reduce(lambda x, y: x + y, diffs) / len(diffs)
